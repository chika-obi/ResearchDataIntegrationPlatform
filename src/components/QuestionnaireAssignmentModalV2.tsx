...
export const QuestionnaireAssignmentModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  const loadEnumerators = async (pid: string) => {
    const { data: members, error: me } = await supabase.from('project_members').select('user_id').eq('project_id', pid).eq('status', 'active');
    if (me) { setEnumerators([]); setError(`Unable to load project enumerators: ${me.message}`); return; }
    const ids = (members || []).map((m: { user_id: string }) => m.user_id).filter(Boolean);
    if (!ids.length) { setEnumerators([]); return; }
    const { data, error: pe } = await supabase.from('profiles').select('id,email,full_name,phone,avatar_url,institution,department,role,status').in('id', ids).eq('role', 'enumerator').eq('status', 'active').order('full_name');
    if (pe) { setEnumerators([]); setError(`Unable to load eligible enumerators: ${pe.message}`); }
    else setEnumerators((data || []) as DbProfile[]);
  };

  useEffect(() => {
    if (!isOpen) return;
    setProjectId(''); setQuestionnaireId(''); setVersionId(''); setEnumeratorId(''); setStartDate(''); setEndDate(''); setError(null); setSuccess(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setError(null);
      const { data, error: e } = await supabase.from('questionnaires').select('*').eq('project_id', projectId).order('created_at', { ascending: false });
      if (cancelled) return;
      if (e) setError(`Unable to load questionnaires: ${e.message}`); else setQuestionnaires((data || []) as DbQuestionnaire[]);
      await loadEnumerators(projectId);
    };
  ...
  const registerEnumerator = async (event: React.FormEvent) => {
    event.preventDefault(); setRegisterError(null); setRegisterMessage(null);
    if (!projectId) { setRegisterError('Select a research project first.'); return; }
    if (!registerName.trim() || !registerEmail.trim()) { setRegisterError('Enumerator name and email are required.'); return; }
    setRegistering(true);
    try {
      const { data } = await supabase.auth.getSession();
      const token = data.session?.access_token;
      if (!token) throw new Error('Authentication error: Please sign in again.');
      const response = await fetch('/api/enumerators', { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ name: registerName.trim(), email: registerEmail.trim(), phone: registerPhone.trim(), projectId }) });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || 'Unable to register enumerator.');
      setRegisterMessage(payload.message || 'Enumerator registered successfully.');
      setRegisterName(''); setRegisterEmail(''); setRegisterPhone('');
      await loadEnumerators(projectId);
      if (payload.enumerator?.id) setEnumeratorId(payload.enumerator.id);
    } catch (e) { setRegisterError(e instanceof Error ? e.message : 'Unable to register enumerator.'); }
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!selectedEnumerator) { setError('Select an active project enumerator.'); return; }
    const { data: auth, error: authError } = await supabase.auth.getUser();
    if (authError || !auth.user) { setError('Authentication error: Please sign in again.'); return; }
    setSaving(true);
    // project_id is intentionally omitted: questionnaire_assignments derives project context through questionnaire_id.
    const { data, error: e } = await supabase.from('questionnaire_assignments').insert({ questionnaire_id: questionnaireId, questionnaire_version_id: versionId, enumerator_id: enumeratorId, assigned_by: auth.user.id, status: 'active', start_date: startDate || null, end_date: endDate || null, permissions: { can_collect: true, can_view_history: true, can_edit_drafts: false } }).select('id').single();
    setSaving(false);
    if (e || !data) {
      if (e?.code === '23505') setError('This questionnaire is already actively assigned to this enumerator.');
  ...
  return <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
    <div className="absolute inset-0 bg-[#161c27]/45 backdrop-blur-xs" onClick={() => !saving && !registering && onClose()} />
    <div className="relative bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden border border-[#c4c6cf]/40 animate-in zoom-in-95">
      ...
      {success ? <div className="p-6 space-y-5"><div className="p-4 rounded-xl bg-[#006a68]/10 border border-[#006a68]/30 flex items-start gap-3"><span className="material-symbols-outlined text-[#006a68] text-2xl">verified</span><div><h3 className="text-sm font-bold text-[#002045]">Questionnaire assigned successfully.</h3><p className="text-xs text-[#43474e] mt-1">The assignment has been saved to Supabase.</p></div></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs"><div className="p-3 bg-[#f9f9ff] rounded-lg border border-[#c4c6cf]/40"><span className="text-[#74777f] block">Questionnaire</span><strong className="text-[#002045]">{success.questionnaire}</strong></div><div className="p-3 bg-[#f9f9ff] rounded-lg border border-[#c4c6cf]/40"><span className="text-[#74777f] block">Version</span><strong className="text-[#002045]">{success.version} · PUBLISHED</strong></div><div className="p-3 bg-[#f9f9ff] rounded-lg border border-[#c4c6cf]/40"><span className="text-[#74777f] block">Enumerator</span><strong className="text-[#002045]">{success.enumerator}</strong></div><div className="p-3 bg-[#f9f9ff] rounded-lg border border-[#c4c6cf]/40"><span className="text-[#74777f] block">Assignment ID</span><code className="text-[#002045] break-all">{success.id}</code></div></div><div className="flex justify-end pt-3 border-t border-[#c4c6cf]/40"><button onClick={onClose} className="px-5 py-2 rounded-lg bg-[#1a365d] text-white text-xs font-semibold">Done</button></div></div> : <form onSubmit={submit} className="p-6 space-y-5 text-xs">
        ...
        <div><div className="flex items-center justify-between gap-3"><label className="block font-semibold text-[#161c27] mb-1">Enumerator *</label><button type="button" onClick={() => { setRegisterOpen(true); setRegisterError(null); setRegisterMessage(null); }} disabled={!projectId || saving} className="text-[11px] font-bold text-[#006a68] hover:text-[#004f4e]">+ Register / Invite Enumerator</button></div><select required value={enumeratorId} onChange={e => setEnumeratorId(e.target.value)} disabled={!projectId || saving} className="w-full p-2.5 rounded-lg border border-[#c4c6cf] focus:border-[#1a365d] outline-none bg-white"><option value="">{enumerators.length ? 'Select enumerator' : 'No active project enumerator found'}</option>{enumerators.map(e => <option key={e.id} value={e.id}>{e.full_name} — {e.email}</option>)}</select><p className="mt-1 text-[11px] text-[#74777f]">Only active enumerator profiles who are active project members are shown.</p></div>
    ...
    {registerOpen && <div className="fixed inset-0 z-[140] flex items-center justify-center p-4"><div className="absolute inset-0 bg-[#161c27]/55 backdrop-blur-xs" onClick={() => !registering && setRegisterOpen(false)} /><div className="relative bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden border border-[#c4c6cf]/40 animate-in zoom-in-95"><div className="bg-[#1a365d] text-white p-5 flex justify-between items-center"><div><h2 className="text-base font-bold">Register Field Enumerator</h2><p className="text-xs text-white/80">Supabase Auth + active project membership</p></div><button disabled={registering} onClick={() => setRegisterOpen(false)} className="text-white/80 hover:text-white p-1"><span className="material-symbols-outlined text-[20px]">close</span></button></div><form onSubmit={registerEnumerator} className="p-5 space-y-4 text-xs">{registerError && <div role="alert" className="p-3 rounded-lg bg-[#ba1a1a]/10 border border-[#ba1a1a]/20 text-[#8b1515]">{registerError}</div>}{registerMessage && <div className="p-3 rounded-lg bg-[#006a68]/10 border border-[#006a68]/30 text-[#005c5a]">{registerMessage}</div>}<div className="p-3 rounded-lg bg-[#f1f3ff] border border-[#c4c6cf]/40"><span className="text-[#74777f]">Project</span><strong className="block text-[#002045] mt-0.5">{projects.find(p => p.id === projectId)?.title || 'Selected project'}</strong></div><div><label className="block font-semibold text-[#161c27] mb-1">Enumerator Full Name *</label><input required value={registerName} onChange={e => setRegisterName(e.target.value)} placeholder="e.g. Nimi Davies" disabled={registering} className="w-full p-2.5 rounded-lg border border-[#c4c6cf] focus:border-[#1a365d] outline-none" /></div><div><label className="block font-semibold text-[#161c27] mb-1">Enumerator Email *</label><input required type="email" value={registerEmail} onChange={e => setRegisterEmail(e.target.value)} placeholder="enumerator@example.com" disabled={registering} className="w-full p-2.5 rounded-lg border border-[#c4c6cf] focus:border-[#1a365d] outline-none" /></div><div><label className="block font-semibold text-[#161c27] mb-1">Contact Phone</label><input type="tel" value={registerPhone} onChange={e => setRegisterPhone(e.target.value)} placeholder="08011111111" disabled={registering} className="w-full p-2.5 rounded-lg border border-[#1a365d] focus:border-[#1a365d] outline-none" /></div><div className="p-3 rounded-xl bg-[#fff7ed] border border-[#fed7aa] text-[#7c2d12]"><strong>Secure provisioning</strong><p className="mt-1">The Supabase secret key stays server-side. The registered account receives enumerator role and active membership for this project.</p></div><div className="pt-3 border-t border-[#c4c6cf]/40 flex justify-end gap-2"><button type="button" onClick={() => setRegisterOpen(false)} disabled={registering} className="px-4 py-2 rounded-lg border border-[#c4c6cf] font-semibold text-[#002045]">Cancel</button><button type="submit" disabled={registering} className="px-5 py-2 rounded-lg bg-[#006a68] text-white font-semibold disabled:opacity-50 flex items-center gap-1.5">{registering ? <><span className="material-symbols-outlined text-[16px] animate-spin">sync</span>Registering...</> : <><span className="material-symbols-outlined text-[16px]">person_add</span>Register Enumerator</>}</button></div></form></div></div>}
  </div>;
};
