...
export const QuestionnaireAssignmentModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setSaving(true);
    const { data, error: e } = await supabase.from('questionnaire_assignments').insert({ project_id: projectId, questionnaire_id: questionnaireId, questionnaire_version_id: versionId, enumerator_id: enumeratorId, assigned_by: auth.user.id, status: 'active', start_date: startDate ? new Date(`${startDate}T00:00:00`).toISOString() : null, end_date: endDate ? new Date(`${endDate}T23:59:59`).toISOString() : null, permissions: { can_collect: true, can_view_history: true, can_edit_drafts: false } }).select('id').single();
    setSaving(false);
    if (e || !data) {
